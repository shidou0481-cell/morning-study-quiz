...
湖・海|瀬戸内海|本州・四国・九州の間|雨が少なく、おだやかな海|325,270`.split('\n').map(x=>{const [category,place,answer,explain]=x.split('|');return{era:'地理',category,place,answer,explain,level:2,type:'choice',kind:'地理・頻出',text:`次の説明に当てはまる${category}は何ですか。\n${explain}`,aliases:[answer]}});
  // 選択肢は必ず同じ分類（川なら川、山脈なら山脈）から作る。
  const geoMapBank=geoPlaces.map(x=>{const peers=geoPlaces.filter(y=>y.category===x.category&&y.place!==x.place);return{...x,options:shuffle([x.place,...shuffle(peers.map(y=>y.place)).slice(0,3)]),answer:x.place}});
