...
湖・海|瀬戸内海|本州・四国・九州の間|雨が少なく、おだやかな海|325,270`.split('\n').map(x=>{const [category,place,answer,explain,pos]=x.split('|');return{era:'地理',category,place,answer,explain,pos,level:2,type:'choice',kind:'白地図・地理',text:`拡大白地図の★が示す${category}は何ですか。`,mapOnly:true}});
  ...
  function mapSVG(q){if(!q.pos)return '';const markers={
    ...
    const [left,top]=(markers[q.place]||'50,50').split(',').map(Number);
    const featureType=q.category==='川'?'river':q.category==='湖・海'?'lake':q.category==='平野'?'plain':'mountain';
    const featureShape={river:'<svg class="feature-mark river" viewBox="0 0 100 100" aria-hidden="true"><path d="M8 88 C22 71 31 79 42 57 S63 31 91 10"/></svg>',lake:'<svg class="feature-mark lake" viewBox="0 0 100 100" aria-hidden="true"><ellipse cx="50" cy="50" rx="27" ry="17"/></svg>',plain:'<svg class="feature-mark plain" viewBox="0 0 100 100" aria-hidden="true"><path d="M17 60 Q29 27 53 34 Q78 25 87 57 Q75 77 51 75 Q27 81 17 60Z"/></svg>',mountain:'<svg class="feature-mark mountain" viewBox="0 0 100 100" aria-hidden="true"><path d="M14 76 L35 33 L49 58 L64 22 L86 74"/><path d="M25 80 L44 42 L57 63 L75 34"/></svg>'}[featureType];
    if(q.mapOnly){const zoom=2.3, mapLeft=50-left*zoom, mapTop=50-top*zoom;return `<div class="map-card"><div class="focus-map" role="img" aria-label="${q.place}のある地域を拡大した都道府県境入り白地図" style="--zoom:${zoom*100}%;--map-left:${mapLeft}%;--map-top:${mapTop}%"><img class="map-base" src="https://raw.githubusercontent.com/geolonia/japanese-prefectures/master/map-mobile.svg" alt=""><span class="map-pin" aria-label="問題の地点">★</span>${featureShape}</div><div class="mini map-note">この地域を拡大しています。都道府県境と、${q.category}の形・位置を手がかりに答えよう。</div><div class="mini">地図素材: Geolonia Japanese Prefectures（GFDL）</div></div>`}
